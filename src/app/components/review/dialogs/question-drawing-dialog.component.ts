...
export interface QuestionDrawingDialogData {
  questionId: string;
  questionLabel: string;
}

export type QuestionDrawingDialogResult = 'saved' | 'deleted' | undefined;

export const QUESTION_DRAWING_DIALOG_INITIAL_WIDTH = '96vw';
export const QUESTION_DRAWING_DIALOG_INITIAL_HEIGHT = '88vh';
export const QUESTION_DRAWING_DIALOG_MAXIMIZED_WIDTH = '100vw';
export const QUESTION_DRAWING_DIALOG_MAXIMIZED_HEIGHT = '100vh';
export const QUESTION_DRAWING_DIALOG_MAXIMIZED_PANEL_CLASS =
  'app-question-drawing-dialog--maximized';

@Component({
  selector: 'app-question-drawing-dialog',
  standalone: true,
  imports: [
    CommonModule,
    MatDialogModule,
  ...
  templateUrl: './question-drawing-dialog.component.html',
  styleUrl: './question-drawing-dialog.component.scss',
})
export class QuestionDrawingDialogComponent implements AfterViewInit, OnDestroy {
  ...
  readonly primaryTools: { kind: DrawingToolKind; icon: string; labelKey: string }[] = [
    { kind: 'rect', icon: 'crop_square', labelKey: 'questionDrawing.toolRect' },
    { kind: 'circle', icon: 'circle', labelKey: 'questionDrawing.toolCircle' },
    { kind: 'triangle', icon: 'change_history', labelKey: 'questionDrawing.toolTriangle' },
    { kind: 'diamond', icon: 'diamond', labelKey: 'questionDrawing.toolDiamond' },
    { kind: 'trapezoid', icon: 'pentagon', labelKey: 'questionDrawing.toolTrapezoid' },
    { kind: 'text', icon: 'text_fields', labelKey: 'questionDrawing.toolText' },
  ];

  readonly secondaryTools: { kind: DrawingToolKind; icon: string; labelKey: string }[] = [
    { kind: 'arrow', icon: 'arrow_forward', labelKey: 'questionDrawing.toolArrow' },
  ];
  ...
  constructor(
    ...
    >,
    @Inject(MAT_DIALOG_DATA) public data: QuestionDrawingDialogData
  ) {}
  ...
  toggleDialogMaximized(): void {
    ...
    if (this.dialogMaximized) {
      this.dialogRef.updateSize(
        QUESTION_DRAWING_DIALOG_MAXIMIZED_WIDTH,
        QUESTION_DRAWING_DIALOG_MAXIMIZED_HEIGHT
      );
      this.dialogRef.addPanelClass(QUESTION_DRAWING_DIALOG_MAXIMIZED_PANEL_CLASS);
    } else {
      this.dialogRef.updateSize(
        QUESTION_DRAWING_DIALOG_INITIAL_WIDTH,
        QUESTION_DRAWING_DIALOG_INITIAL_HEIGHT
      );
      this.dialogRef.removePanelClass(QUESTION_DRAWING_DIALOG_MAXIMIZED_PANEL_CLASS);
    }
    this.dialogRef.updatePosition();
    requestAnimationFrame(() => this.syncCanvasToWrapSize());
  }
  ...
  private syncCanvasToWrapSize(): void {
  ...
  save(): void {
    ...
    this.api
      .saveQuestionDessin(this.data.questionId, dessin)
      ...
      .subscribe({
        ...
        error: () => {
          this.saveError = this.translate.instant('questionDrawing.saveError');
        },
      });
  }
  ...
  deleteDessin(): void {
    ...
    this.saveError = '';
    this.api
      .deleteQuestionDessin(this.data.questionId)
      ...
      .subscribe({
        ...
        error: () => {
          this.saveError = this.translate.instant('questionDrawing.deleteError');
        },
      });
  }
  ...
  private loadDessin(): void {
    this.loading = true;
    this.loadError = '';
    this.api.getQuestionDessin(this.data.questionId).subscribe({
      next: (res) => {
      ...
      error: () => {
        this.loadError = this.translate.instant('questionDrawing.loadError');
        this.dessinFetchDone = true;
        this.loading = false;
      },
    });
  ...
  private onCanvasDoubleClick(opt: {
    ...
    const label = this.createEditableText(
      this.translate.instant('questionDrawing.defaultLabel'),
      {
        left: center.x,
    ...
    switch (kind) {
      ...
      case 'text': {
        const text = this.createEditableText(
          this.translate.instant('questionDrawing.defaultText'),
          {
            left: x,
            top: y,
            fontSize: LABEL_FONT_SIZE_DEFAULT,
